...
class Downloader {
    ...
    async load(url, name) {
        ...
        if (!this.isRemoteUrl(url)) {
            ...
            [err, res] = await uni.downloadFile({
                url: url
            });
        ...
        // #ifdef MP-ALIPAY
        return [{
            errMsg: 'plat not support download!'
        }, null]; //在支付宝小程序中，不支持saveImageToPhotosAlbum，需要使用其它的方式实现下载
        // #endif

        // #ifdef APP-PLUS || MP
        [err, res] = await uni.saveImageToPhotosAlbum({
